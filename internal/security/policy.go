...
func DefaultCommandPolicy() CommandPolicy {
	return CommandPolicy{
		...
		Rules: []CommandRule{
			...
			{Name: "uname", Pattern: `^uname(\s+-a)?$`, Enabled: true},
			...
			{Name: "systemctl_status", Pattern: `^systemctl\s+status\s+[^\s]+(\s+--no-pager)?$`, Enabled: true},
			{Name: "systemctl_is_active", Pattern: `^systemctl\s+is-active\s+[^\s]+$`, Enabled: true},
